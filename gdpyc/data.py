...
def parse_DL(mapfile, outfile):
    ...
    chdu = fits.BinTableHDU.from_columns(
        fits.ColDefs([col1]), header=header, name="LAB"
    )
...
def parse_LAB(mapfile, outfile):
    ...
    col1 = fits.Column(
        name="TEMPERATURE", format="D", array=nhmap.reshape(nrows * ncols)
    )
    chdu = fits.BinTableHDU.from_columns(
        fits.ColDefs([col1]), header=header, name="LAB"
    )
...
def parse_HI4PI(mapfile, outfile):
    ...
    col1 = fits.Column(name="TEMPERATURE", format="D", array=nhmap)
    chdu = fits.BinTableHDU.from_columns(
        fits.ColDefs([col1]), header=header, name="HI4PI"
    )
    chdu.writeto(outfile, overwrite=True)
...
def parse_Planck13(mapfile, outfile):
    ...
    chdu = fits.BinTableHDU.from_columns(
        fits.ColDefs([col1]), header=header, name="Planck13"
    )
    chdu.writeto(outfile, overwrite=True)
...
def get_map(maplabel, data_dir):
    ...
    maps = {
        "DL": "https://lambda.gsfc.nasa.gov/data/foregrounds/combined_nh/lambda_combined_nh.fits",
